...
class CapsuleLowRankBilinearLayer(nn.Module):
    def __init__(
        self,         
        embed_dim, 
        att_type,
        ...
        super(CapsuleLowRankBilinearLayer, self).__init__()
        self.encoder_attn = CapsuleLowRank(
            embed_dim = embed_dim, 
            att_type = att_type, 
            att_heads = att_heads, 
...
class CapsuleLowRankBilinearEncBlock(nn.Module):
    def __init__(
        self, 
        embed_dim, 
        att_type,
        ...
        super(CapsuleLowRankBilinearEncBlock, self).__init__()
        
        self.layers = nn.ModuleList([])
        self.bifeat_emb = nn.ModuleList([])
        self.layer_norms = nn.ModuleList([]) 
        for _ in range(layer_num):
            sublayer = CapsuleLowRankBilinearLayer( 
                embed_dim = embed_dim, 
                att_type = att_type,
                att_heads = att_heads,
                att_mid_dim = att_mid_dim,
...
class CapsuleLowRankBilinearDecBlock(nn.Module):
    def __init__(
        self, 
        embed_dim, 
        att_type,
        ...
        super(CapsuleLowRankBilinearDecBlock, self).__init__()
